# -*- coding: utf-8 -*-
"""
Created on Fri Oct  9 11:19:50 2020

@author: Juan Esteban Ramirez, Maria Paula Rey
"""
import numpy as np
import math 
...
def ray_tracing(width, height, rayo, so1, n1, si1, sL, obj, pixels):
    ...
    a9 = (1 - (D4*d2)/nl)
    a10 = -D3-D4+(D3*D4*d2/nl)
    a11 = dl/nl
    a12 = (1 - (D3*d2)/nl)
    A3 = np.array([[a9,a10],[a11,a12]])
    ...
    Prop4 = np.array([[1,0],[sE/n5,1]])
  
    #Creacion de los rayos mediante el cambio de coordenadas.(Expropiado)
    for i in range(width):
        for j in range(height):
            ...
            V_salida = Prop2.dot(E2.dot(Prop1.dot(A3.dot(Prop4.dot(A2.dot(Prop3.dot(A1.dot(Prop1.dot(E1.dot(Prop1.dot(V_entrada)))))))))))
...
#Después
si1 = 1              #Distancia desde el vertice del lente 2 hasta el plano imagen
#si = 0.751
n5 = 1               #Indice de refracción del aire 
sL= -f               #Distancia desde el vertice del lente 1 hasta el vertice del lente 2
sE= f2+(0.1*f2)
#so = 0.5
so1 = 1              #Distancia desde el verticce del lente hasta el plano objeto
#so = 0.5
